...
We compare the results of our vectorized version of the function fdp_fp_integrate.
...
energy_vec = torch.tensor(energy_vec)
intervals_mat = torch.tensor([[0, energy_max]])
coeff_mat = torch.tensor(popt_0)[None]
powers_mat = torch.tensor(powers)[None]

fp_calculated = fdp_fp_integrate(energy_vec, intervals_mat, coeff_mat, powers_mat, relativistic_correction)

plt.figure()
plt.plot(energy_vec, fp_vec, 'r.', label="original")
plt.plot(energy_vec, fp_calculated, 'b', label="calculated")
plt.legend()
plt.savefig("tst_fp_calculated_vectorized.png")
