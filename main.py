...
def add_nanoBragg_spot(SIM, verbose=True):
    max_I = 0.0
    i = 0
    floatimage = SIM.raw_pixels.begin()

    if(verbose): 
        print("TESTING sincg(1,1)= %f\n",sincg(1,1))
...
def loop_over_ssubpixels():
                {
                for(subF=0;subF<oversample;++subF)
                    loop_over_fsubpixels

                /* end of sub-pixel y loop */
            }

def loop_over_fsubpixels():
                {
...
def loop_over_phi(): # not needed for XFEL
                        {
