...
def set_nanoBragg_params(params, basic_params, sfall_channels, num_pixels_x, num_pixels_y, simulate=True, use_background=True):
    """
    finish getting the simulation parameters from nanoBragg
    can optionally simulate a diffraction image by setting simulate=True
    """
    detpixels_slowfast=(num_pixels_x, num_pixels_y)
...
def forward_sim_pytorch(params, basic_params, Fhkl_mat_vec, nanoBragg_params, noise_params, 
                    fluence_background, hkl_ranges, device, num_pixels_x, num_pixels_y, use_background=True):
    ...
    spixels = num_pixels_x
    fpixels = num_pixels_y
    ...
    for x in range(len(flux)):
        ...
        raw_pixels_x = add_torchBragg_spots(spixels, fpixels, phisteps, mosaic_domains, oversample, pixel_size,
                                            roi_xmin, roi_xmax, roi_ymin, roi_ymax, maskimage, detector_thicksteps,
                                            spot_scale, fluence, detector_thickstep, Odet, fdet_vector, sdet_vector, 
                                            odet_vector, pix0_vector, curved_detector, distance, beam_vector, 
                                            close_distance, point_pixel, detector_thick, detector_attnlen, sources,
                                            source_X, source_Y, source_Z, source_lambda, dmin,phi0, phistep,
                                            a0, b0, c0, ap, bp, cp, spindle_vector, mosaic_spread_deg*math.pi/180,
    ...
    if use_background:
        ...
        background_pixels_water, invalid_pixel = add_background(oversample, override_source, sources, spixels, fpixels, pixel_size,
                                                                roi_xmin, roi_xmax, roi_ymin, roi_ymax, detector_thicksteps,
                                                                ...
                                                                detector_thickstep_water, Odet_water, fdet_vector, sdet_vector, odet_vector, 
                                                                pix0_vector, curved_detector, distance, beam_vector, close_distance, 
                                                                point_pixel, detector_thick, detector_attnlen, source_I, source_X, source_Y, 
                                                                source_Z, source_lambda_background, stol_of_water, stols_water, Fbg_of_water, 
                                                                nopolar, polarization, polar_vector, verbose=True, use_numpy=False, device=device,
                                                                )
        ...
        background_pixels_air, invalid_pixel = add_background(oversample, override_source, sources, spixels, fpixels, pixel_size,
                                                              roi_xmin, roi_xmax, roi_ymin, roi_ymax, detector_thicksteps,
                                                              fluence_background, amorphous_molecules_air, 
                                                              Fmap_pixel, # bool override: just plot interpolated structure factor at every pixel, useful for making absorption masks
